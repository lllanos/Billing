            ...
            <table class="table table-striped table-hover table-bordered zui-table">
              <thead>
                <tr>
                  ...
                  <th>{{trans('forms.expedient_ppal_elec')}}</th>
                  <th>{{trans('forms.description')}}</th>
                  <th>{{trans('forms.obra')}}</th>
              ...
              <tbody class="tbody_js">
                @foreach ($redeterminaciones as $key => $redeterminacion)
                  <tr>
                    <td class="text-center">
                      @if($redeterminacion->a_termino)
                        <span>
                          <i class="fa fa-check-circle text-success"
                           data-toggle="tooltip" data-placement="top" title="{{ trans('redeterminaciones.a_termino')}}"></i>
                        </span>
                      @else
                        <span>
                          <i class="fa fa-times-circle text-danger"
                           data-toggle="tooltip" data-placement="top" title="{{ trans('redeterminaciones.no_a_termino')}}"></i>
                        </span>
                      @endif
                    </td>
                    <td>{{ $redeterminacion->salto->obra->contrato->expedientes }}</td>
                    <td>{{ $redeterminacion->user_contrato->descripcion }}</td>
                    <td>{{ $redeterminacion->obra->categoria_obra->nombre }}</td>
                    <td>{{ $redeterminacion->created_at }}</td>
                    <td>
                    ...
                    <td>
                      <div class="contenedor_badges_estado_contrato">
                        ...
                        @endif
                        <span class="m-0 badge badge-referencias container_estado_redeterminacion @if($redeterminacion->en_curso) badge_esperando_estado @endif" style="background: #{{$redeterminacion->estado_nombre_color['color']}}" >
                          <span>{{$redeterminacion->estado_nombre_color['nombre']}}</span>
                        </span>
                      </div>
                    </td>
                    <td>{{ $redeterminacion->ultimo_movimiento }}</td>
                    <td class="actions-col noFilter">
                      <div class="dropdown dd-on-table" data-toggle="tooltip" data-placement="left" title="{{ trans('index.opciones')}}">
                        ...
                        <ul class="dropdown-menu pull-right">
                          <li><a href="{{ route('solicitudes.ver', ['id' => $redeterminacion->id]) }}"><i class="glyphicon glyphicon-eye-open"></i> {{ trans('index.ver')}}</a></li>
                      </ul>
                    </div>
                    </td>
                  </tr>
