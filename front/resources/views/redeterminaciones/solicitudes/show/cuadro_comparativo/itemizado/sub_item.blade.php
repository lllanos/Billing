  <div class="panel-group colapsable_sub m-0 pt-1" id="accordion_sub_a_pre_{{$subItem->id}}" data-id='{{$subItem->id}}' role="tablist" aria-multiselectable="true">
    <div class="panel panel-default" id="pepe">
      @if ($subItem->is_hoja)
        <div class="panel-heading panel_heading_collapse p-0" role="tab" id="headingOne_sub_a_pre_{{$subItem->id}}">
          <h4 class="panel-title titulo_collapse titulo_collapse_small panel_heading_{{$tab}} m-0 panel_title_btn">
            @php($item = $subItem)
            @php ($item_cuadro_comparativo = $cuadro_comparativo)
            {{-- @php ($item_cuadro_comparativo = $cuadro_comparativo->precioRedeterminadosDeItem($item->id)) --}}
            @include('redeterminaciones.solicitudes.show.cuadro_comparativo.itemizado.fila', ['header' => false])
            
              @if($item->is_hoja) @php ($item_cuadro_comparativo = $cuadro_comparativo->getItemCuadroComparativo($item)) @endif
              <div class="dropdown container_btn_action dato-opciones" data-toggle="tooltip" data-placement="bottom" title="@trans('index.opciones')">
                ...
                <ul class="dropdown-menu pull-right">
                  <li><a href="{{route('cuadroComparativo.item.ver', ['id' => $item_cuadro_comparativo->id])}}"><i class="glyphicon glyphicon-eye-open"></i> @trans('index.ver') @trans('redeterminaciones.cuadro_comparativo.analisis_item')</a></li>
                </ul>
              </div>
          </h4>
        </div>
      @else
        ...
        <div id="collapse_sub_cuad_{{$subItem->id}}" class="panel-collapse collapse in" role="tabpanel" aria-labelledby="headingOne_sub_a_pre_{{$subItem->id}}">
