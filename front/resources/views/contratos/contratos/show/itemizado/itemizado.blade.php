<!--Panel-->
  <div class="panel-body panel_con_tablas_y_sub_tablas p-0 contenedor_all_tablas">
    ...
    @if(isset($itemizado) &&
      ...
      <div class="panel-group colapsable_top mt-1" id="accordion{{$sufijo}}{{$keyItemizado}}" role="tablist" aria-multiselectable="true">
        <div class="panel panel-default">
          {{-- Encabezado --}}
          <div class="panel-heading panel_heading_padre p-0 panel_heading_collapse" role="tab" id="heading{{$sufijo}}{{$keyItemizado}}">
            <h4 class="panel-title titulo_collapse m-0 panel_title_btn">
              @if(isset($isExcel))
                DATOS EXCEL
              @else
                <a class="btn_acordion datos_as_table collapse_arrow" role="button" data-toggle="collapse" data-parent="#accordion{{$sufijo}}{{$keyItemizado}}" href="#collpapse{{$sufijo}}{{$keyItemizado}}" aria-expanded="true" aria-controls="collpapse{{$sufijo}}{{$keyItemizado}}">
          ...
          <div id="collpapse{{$sufijo}}{{$keyItemizado}}" class="panel-collapse collapse in" role="tabpanel" aria-labelledby="headingOne_sub{{$sufijo}}{{$keyItemizado}}">
            <div class="panel-body panel_sub_tablas pl-0 pt-1 pr-0 pb-0 scrollable-collapse">
              @if(count($itemizado->items_nivel_1) > 0)
                ...
                @if(isset($fromCronograma))
                  <div class="panel-body panel_sub_tablas p-0">
                    <div class="panel panel-default">
                      <div class="panel-heading panel_heading_collapse p-0" role="tab" id="headingOne_sub_heading">
                ...
                @foreach($itemizado->items_nivel_1 as $keyItem => $level1)
                  <div class="panel-body panel_sub_tablas p-0">
                    <div class="panel panel-default">
                            ...
                            @if($itemizado->borrador == 1 && !isset($fromCronograma))
                              <div class="dropdown container_btn_action dato-opciones" data-toggle="tooltip" data-placement="bottom" title="@trans('index.opciones')">
                                ...
                                <ul class="dropdown-menu pull-right">
                                  <li class="btn_edit_itemizado_item" data-itemizado="{{$keyItemizado}}" data-padre_id="{{$level1->padre_id}}" data-id="{{$level1->id}}" data-nivel="{{$level1->nivel}}" data-toggle="modal" data-target="#itemizadoAddModal"><a><i class="fa fa-pencil" aria-hidden="true"></i> {{trans('index.editar')}}</a></li>                                  
                                  @if (sizeof($level1->child) == 0)
                                    <li>
                                      <a class="btn-confirmable"
                                         data-body="{{trans('contratos.confirmacion.delete-itemizado', ['item' => $level1->descripcion])}}" data-action="{{route('itemizado.deleteItem', ['item_id' => $level1->id ])}}" data-si="@trans('index.si')" data-no="@trans('index.no')">
                      ...
                      <div id="collapse_sub{{$sufijo}}{{$level1->id}}" class="aaa panel-collapse collapse in" role="tabpanel" aria-labelledby="headingOne_sub{{$sufijo}}{{$level1->id}}">
                        <div class="panel-body panel_sub_tablas p-0 iaaa">
                          @if(count($level1->child))
