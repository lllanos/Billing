...
class ReConfirmUserNotification extends Notification
{
    ...
    {
      if($notifiable->usuario_sistema == 1) {
        ...
        return (new MailMessage)
  		        ->subject(trans(trans('mail.eby').' - '.trans('mail.front.reconfirmar_usuario.subject')))
  		        ->greeting(trans('mail.front.reconfirmar_usuario.hola'))
              ->line(trans('mail.front.reconfirmar_usuario.cuerpo'))
              ->line(trans('mail.front.reconfirmar_usuario.con_contrasenia').$this->password)
              ->action(trans('mail.front.reconfirmar_usuario.boton'), url(config('app.url').route('register.verify', array($this->token), false)))
              ->line(trans('mail.front.reconfirmar_usuario.mensaje_despedida'));
