  ...
  <div class="row">
    <div class="col-md-12 col-sm-12">
      <ol class="breadcrumb">
        ...
        <li class="active">{!!trans('index.reporte_indices_fuentes')!!}</li>
      </ol>
      <div class="page-header">
        <h3 class="page_header__titulo titulo__fecha__front">
          <div class="titulo-header__input titulo-header__input__front">
            <div class="titulo__contenido"> @trans('index.reporte_indices_fuentes') @trans('index.en') <span id="moneda_title">{{$moneda->nombre_simbolo}}</span> </div>
            <div class="titulo__input titulo__input_fecha_fecha">
              <select class="form-control chosen_title" name="select_anio" id="select_anio">
                @foreach($anios as $keyAnio => $valueAnio)
                  <option value="{{ $keyAnio }}" @if($keyAnio == $selected) selected @endif
                  data-route="{{route('publicaciones.getHtmlTablafuentesIndices', ['de' => $valueAnio['de'], 'a' => $valueAnio['a'], 'moneda_id' => ':moneda_id'])}}"
                  data-excelroute="{{ route('publicaciones.export.exportarFuentes', ['de' => $valueAnio['de'], 'a' => $valueAnio['a'], 'moneda_id' => ':moneda_id'])}}">
                  {{ $valueAnio['show'] }} </option>
                @endforeach
              </select>
            </div>
          </div>
            ...
            <div class="">
              <div class="dropdown dd-on-table" data-toggle="tooltip" data-placement="left" title="">
                ...
                <ul class="dropdown-menu pull-right multi-level" role="menu" aria-labelledby="dropdownMenu">
                  <li>
                    <a href="{{ route('publicaciones.reporteIndices') }}">{{trans('publicaciones.ver_valores')}}</a>
                  </li>
    ...
    <!--Input file excel con 1 form-->
      <div class="input-group rounded col-xs-12 col-sm-12 col-md-12 mb-_5 badges_vr__input_excel" role="group" aria-label="...">
        ...
        <div class="col-xs-12 col-sm-6 col-md-6 contenedor_input_dos_btns mb-_5">
          <form class="form_excel" method="POST"
            data-action="{{ route('publicaciones.export.exportarFuentes', ['de' => $anios[$selected]['de'], 'a' => $anios[$selected]['a'], 'moneda_id' => $moneda_id]) }}"
            id="form_excel"
          >
            {{ csrf_field() }}
          ...
          <input type="text" class="search-input form-control input_dos_btns"
            name="search_input_no_post" id="search_input_no_post" value =""
            placeholder="@trans('forms.busqueda_placeholder')"
            aria-label="{{trans('index.input')}} @trans('index.buscar')"
          >
          <span class="input-group-btn">
            <button type="submit" id="search_button_no_post" class="btn btn-info btn-filter-toggle"
              data-toggle="tooltip" data-placement="bottom" title="@trans('index.buscar')" aria-label="@trans('index.buscar')"
            >
