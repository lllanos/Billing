                                      ...
                                      <thead>
                                          <tr>
                                            ...
                                            @foreach($ids_publicaciones as $keyPublicacion => $valuePublicacion)
                                              ...
                                              {{trans('date.mes_reducido.'. $valuePublicacion['mes']) }}</th>
                                            @endforeach
                                          </tr>
                                      </thead>
                                      <tbody class="tbody_tooltip">
                                        @foreach($subcategoria[0] as $keyIndice => $valor_indice)
                                          <tr class="@if($valor_indice->no_se_publica) indice_no_se_publica @endif">
                                            <td class="tb_indice_nro text-center">
                                              @if(!isset($isExcel))
                                                {{$valor_indice->nro}}
                                              @endif
                                            </td>
                                            ...
                                            @foreach($ids_publicaciones as $keyPublicacion => $valuePublicacion)
                                              <td class="
                                              @if(!$valuePublicacion['publicado'] || $valor_indice->no_se_publica) no-publicado @endif
                                                tb_meses_reporte text-right">
