            ...
            <div class="titulo__input">
              <select class="form-control chosen_title" name="select_anio" id="select_anio">
                @foreach($anios as $keyAnio => $valueAnio)
                  <option value="{{ $keyAnio }}" @if($selected_anio == $valueAnio) selected @endif
                    data-route="{{route('publicaciones.getHtmlTablareporteIndices', ['anio' => $valueAnio, 'moneda_id' => ':moneda_id'])}}"
                    data-excelroute="{{ route('publicaciones.export.exportarIndices', ['anio' => $valueAnio, 'moneda_id' => ':moneda_id']) }}">
                  {{ $valueAnio }} </option>
                @endforeach
              </select>
            </div>
    ...
    <!--Input file excel con 1 form-->
      <div class="input-group rounded col-xs-12 col-sm-12 col-md-12 mb-_5 badges_vr__input_excel" role="group" aria-label="...">
        ...
        <span class="input-group-btn">
          <button type="submit" id="search_button_no_post" name="search_button_no_post" class="btn btn-info btn-filter-toggle" data-toggle="tooltip" data-placement="bottom" title="@trans('index.buscar')" aria-label="@trans('index.buscar')">
