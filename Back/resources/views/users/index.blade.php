          ...
          <div class="buttons-on-title">
            @permissions(('user-create'))
              <div class="button_desktop">
                <a class="btn btn-success pull-right" href="{{route('seguridad.users.create')}}">
      ...
      <div class="input-group rounded col-xs-12 col-sm-12 col-md-12 mb-_5" role="group" aria-label="...">
        <div class="col-xs-12 col-sm-offset-6 col-sm-6 col-md-offset-6 col-md-6 contenedor_input_dos_btns mb-_5">
          @permissions(('user-export'))
            <form class="form_excel" method="POST" data-action="{{route('seguridad.export.users')}}" id="form_excel">
              {{ csrf_field() }}
                ...
                <tbody class="tbody_js">
                  @foreach($users as $key => $user)
                    <tr id="user_{{$user->id}}" @if(!$user->habilitado) class="disabled_tr" @endif>
                      ...
                      @if(!Auth::user()->usuario_causante)
                        <td>
                          <span class="badge" style="background-color:#{{ $user->causante_nombre_color['color'] }};">
                            {{ $user->causante_nombre_color['nombre'] }}
                          </span>
                        </td>
                      @endif
                        ...
                        <div class="dropdown dd-on-table" data-toggle="tooltip" data-placement="left" title="@trans('index.opciones')">
                          ...
                          <ul class="dropdown-menu pull-right">
                            @permissions(('user-edit'))
                              <li><a href="{{ route('seguridad.users.edit', $user->id) }}"><i class="glyphicon glyphicon-pencil"></i> {{ trans('index.editar')}}</a></li>
                            @endpermission
                            @if(($user->habilitado && Auth::user()->can('user-disable')))
                              <li>
                                <a class="eliminar btn-confirmable-prevalidado"
                                data-prevalidacion="{{ route('seguridad.users.preToggleHabilitar', ['id' => $user->id, 'accion' => 'disable']) }}"
                                data-body="{{trans('index.confirmar_habilitar.usuario_disable', ['nombre' => $user->apellido_nombre])}}"
                                data-action="{{ route('seguridad.users.toggleHabilitado', ['id' => $user->id, 'accion' => 'disable']) }}"
                                data-si="@trans('index.si')" data-no="@trans('index.no')">
                                  <i class="glyphicon glyphicon-user"></i> @trans('index.disable')
                                </a>
                              </li>
                            @elseif(!$user->habilitado && Auth::user()->can('user-enable'))
                              <li>
                                <a class="eliminar btn-confirmable-prevalidado"
                                data-prevalidacion="{{ route('seguridad.users.preToggleHabilitar', ['id' => $user->id, 'accion' => 'enable']) }}"
                                data-body="{{trans('index.confirmar_habilitar.usuario_enable', ['nombre' => $user->apellido_nombre])}}"
                                data-action="{{ route('seguridad.users.toggleHabilitado', ['id' => $user->id, 'accion' => 'enable']) }}"
                                data-si="@trans('index.si')" data-no="@trans('index.no')">
