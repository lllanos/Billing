  <div class="panel-group colapsable_sub m-0 pt-1" id="accordion_sub{{$sufijo}}{{$subItem->id}}" data-id='{{$subItem->id}}' role="tablist" aria-multiselectable="true">
    <div class="panel panel-default" id="pepe">
      @if ($subItem->is_hoja)
        <div class="panel-heading panel_heading_collapse p-0" role="tab" id="headingOne_sub{{$sufijo}}{{$subItem->id}}">
          <h4 class="panel-title titulo_collapse titulo_collapse_small panel_heading_{{$tab}} m-0 panel_title_btn">
           @if(!isset($fromCronograma) && $itemizado->borrador == 1) <span class="@if($tab > 1 && $tab < 3) child_handle @elseif($tab > 3) sub_child_handle @else parent_handle @endif"></span> @endif
            @if(isset($isExcel))
              DATOS EXCEL
            @else
              @php ($item = $subItem)
      ...
      @else
        <div class="panel-heading panel_heading_collapse p-0" role="tab" id="headingOne_sub{{$sufijo}}{{$subItem->id}}">
          <h4 class="panel-title titulo_collapse titulo_collapse_small panel_heading_{{$tab}} m-0 panel_title_btn">
           @if(!isset($fromCronograma) && $itemizado->borrador == 1)<span class="@if($tab > 1)child_handle @else parent_handle @endif "></span> @endif
            @if(isset($isExcel))
              DATOS EXCEL
            @else
              @php ($item = $subItem)
