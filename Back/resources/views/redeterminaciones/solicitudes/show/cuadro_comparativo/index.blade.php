        ...
        <div class="col-md-12">
            <ol class="breadcrumb">
                ...
                @if($cuadro_comparativo->solicitud->en_curso)
                    <li><a href="{{route('solicitudes.redeterminaciones_en_proceso')}}">@trans('forms.sol_redeterminaciones_en_proceso')</a>
                    </li>
                @else
                    <li><a href="{{route('solicitudes.redeterminaciones_finalizadas')}}">@trans('forms.sol_redeterminaciones_finalizadas')</a>
                    </li>
                @endif
                <li><a href="{{route('solicitudes.ver', ['id' => $cuadro_comparativo->solicitud_id])}}">@trans('index.ver')
                        @trans('index.redeterminacion')</a></li>
                <li class="active"> @trans('sol_redeterminaciones.cuadro_comparativo') @trans('index.de')
                    @trans('forms.solicitud') {{$cuadro_comparativo->solicitud->salto->moneda_mes_anio}} </li>
            </ol>
            <div class="page-header">
                <h3 class="page_header__titulo">
                    <div class="titulo__contenido">
                        @trans('sol_redeterminaciones.cuadro_comparativo') @trans('index.de')
                        @trans('forms.solicitud') {{$cuadro_comparativo->solicitud->salto->moneda_mes_anio}}
                    </div>
            ...
            <div class="row">
                <div class="col-md-12">
                            ...
                            <div class="panel-heading p-0 panel_heading_collapse" role="tab" id="heading_Itemizado">
                                <h4 class="panel-title titulo_collapse m-0">
                                    <a class="btn_acordion collapse_arrow" role="button" data-toggle="collapse" data-parent="#accordion_Itemizado" href="#collapse_Itemizado" aria-expanded="true" aria-controls="collapse_Itemizado">
                                        ...
                                        @trans('sol_redeterminaciones.cuadro_comparativo')
                                    </a>
                                </h4>
                            </div>
                    ...
                    @if(config('custom.test_mode') == 'true')
                        @if(count($cuadro_comparativo->items_cuadro_ocultos) > 0)
                            <span class="error-redeterminacion"> SOLO PARA MODO DEBUG:</span>
                            <ul>
                                @foreach ($cuadro_comparativo->items_cuadro_ocultos as $keyItemCuadro => $valueItemCuadro)
                                    <li>
                                        <a href="{{route('cuadroComparativo.item.ver', ['id' => $valueItemCuadro->id])}}">
                                            @trans('index.ver')
                                            @trans('redeterminaciones.cuadro_comparativo.analisis_item')
                                            @trans('index.de') "{{$valueItemCuadro->item->descripcion}}
                                            ({{$valueItemCuadro->item->codigo}})"</a></li>
                                @endforeach
                            </ul>
                        @endif
                    @endif
